...
const CreateEmployeeSchema = z.object({
  ...
  off_rota: z.boolean().default(false),
});

const UpdateEmployeeSchema = CreateEmployeeSchema
  .partial()
  .omit({ password: true })
  .extend({ off_rota: z.boolean().optional() });
...
router.get('/:id/availability', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const rows = await db('availability').where({ employee_id: req.params.id }).orderBy('day_of_week');
    res.json({ data: rows });
  } catch (err) { next(err); }
});
...
router.put('/:id/availability', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await db.transaction(async (trx) => {
      ...
      if (rows.length > 0) await trx('availability').insert(rows.map(r => ({ ...r, employee_id: employeeId })));
    });
