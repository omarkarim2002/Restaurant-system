...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const isActive = (path: string) => {
    ...
    if (path === '/rota') return location.pathname === '/rota' || location.pathname === '/rota-config' || location.pathname === '/time-off';
    if (path === '/bookings') return location.pathname === '/bookings' || location.pathname === '/bookings/analytics';
            ...
            <React.Fragment key={item.path}>
              ...
              {item.path === '/rota' && (
                <div style={{ paddingLeft: '20px' }}>
                  <button
                    className={`nav-item${location.pathname === '/rota-config' ? ' active' : ''}`}
                    onClick={() => navigate('/rota-config')}
                    style={{ fontSize: '12px', opacity: 0.8 }}>
                    <svg className="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ width: '14px', height: '14px' }}>
                      <path d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                    </svg>
                    Config
                  </button>
                  <button
                    className={`nav-item${location.pathname === '/time-off' ? ' active' : ''}`}
              ...
              {item.path === '/bookings' && (
                <div style={{ paddingLeft: '20px' }}>
                  <button
                    className={`nav-item${location.pathname === '/bookings/analytics' ? ' active' : ''}`}
