...
import { format, addDays, startOfWeek, addWeeks, subWeeks, parseISO, isBefore, startOfDay } from 'date-fns';
...
import { ConfirmFinishModal } from '../components/rota/ConfirmFinishModal';
...
function useShiftActuals(scheduleId: string) {
  return useQuery({
    queryKey: ['shift-actuals', scheduleId],
    queryFn: () => api.get(`/shift-actuals?schedule_id=${scheduleId}`).then(r => r.data.data),
    enabled: !!scheduleId,
    staleTime: 30_000,
  });
}

function useConfirmFinish(scheduleId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: any) => api.post('/shift-actuals', body).then(r => r.data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['shift-actuals', scheduleId] });
      qc.invalidateQueries({ queryKey: ['wages'] });
    },
  });
}

function useUnconfirmFinish(scheduleId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (assignmentId: string) => api.delete(`/shift-actuals/${assignmentId}`).then(r => r.data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['shift-actuals', scheduleId] });
      qc.invalidateQueries({ queryKey: ['wages'] });
    },
...
export function RotaPage() {
  ...
  const [confirmMove, setConfirmMove] = useState<any | null>(null);
  const [confirmFinishAssignment, setConfirmFinishAssignment] = useState<any | null>(null);
  ...
  const { data: actuals = [] } = useShiftActuals(currentSchedule?.id || '');
  const removeAssignment = useRemoveAssignment(currentSchedule?.id || '');
  const confirmFinish = useConfirmFinish(currentSchedule?.id || '');
  const unconfirmFinish = useUnconfirmFinish(currentSchedule?.id || '');

  const weekDates = Array.from({ length: 7 }, (_, i) => addDays(selectedWeek, i));

  // Build actuals map: assignment_id → actual
  const actualsMap: Record<string, any> = {};
  for (const a of actuals) actualsMap[a.shift_assignment_id] = a;

  // Group assignments by date, sorted by start time
  ...
  for (const key of Object.keys(assignmentsByDate)) {
    assignmentsByDate[key].sort((a, b) => {
      const aM = a.start_time ? parseInt(a.start_time.replace(':', '')) : 0;
      const bM = b.start_time ? parseInt(b.start_time.replace(':', '')) : 0;
      return aM - bM;
    });
  }
  ...
  // Count unconfirmed past shifts for banner
  const unconfirmedPastCount = allAssignments.filter((a: any) => {
    const shiftDate = startOfDay(parseISO(format(new Date(a.shift_date), 'yyyy-MM-dd')));
    return isBefore(shiftDate, today) && !actualsMap[a.id]?.is_confirmed;
  }).length;
  ...
  function selectAll() { setSelectedIds(new Set(allAssignments.map((a: any) => a.id))); }
  ...
  async function executeBulkDelete() {
    ...
    try {
      for (const a of selectedAssignments) await schedulesApi.removeAssignment(currentSchedule!.id, a.id);
      qc.invalidateQueries({ queryKey: ['schedules', currentSchedule!.id] });
      clearSelection();
    } catch { alert('Some shifts could not be removed.'); }
  ...
  async function executeBulkMove() {
    ...
    for (const a of selectedAssignments) {
      try {
        ...
        await schedulesApi.addAssignment(currentSchedule.id, { employee_id: a.employee_id, shift_id: a.shift_id, shift_date: bulkMoveDate });
      } catch (err: any) { errors.push(`${a.first_name} ${a.last_name}: ${err.response?.data?.error || 'conflict'}`); }
    }
    qc.invalidateQueries({ queryKey: ['schedules', currentSchedule.id] });
    clearSelection();
    if (errors.length) alert(`Some shifts could not be moved:\n${errors.join('\n')}`);
    setBulkMoving(false);
  ...
  // ── Drag ─────────────────────────────────────────────────────────────────────
  function handleDragStart(e: React.DragEvent, a: any) {
  ...
  function handleDrop(e: React.DragEvent, toKey: string) {
    ...
    if (fromDate === toKey || closedDates.has(toKey)) return;
    setConfirmMove({ assignmentId: a.id, employeeName: `${a.first_name} ${a.last_name}`, shiftName: a.shift_name, fromDate, toDate: toKey, scheduleId: currentSchedule.id, employeeId: a.employee_id, shiftId: a.shift_id });
  ...
  async function executeMove() {
    ...
    try {
      ...
      await schedulesApi.addAssignment(confirmMove.scheduleId, { employee_id: confirmMove.employeeId, shift_id: confirmMove.shiftId, shift_date: confirmMove.toDate });
    ...
    } catch (err: any) { alert(err.response?.data?.error || 'Could not move shift.'); }
    finally { setMoving(false); setConfirmMove(null); }
  }

  // ── Confirm finish time ───────────────────────────────────────────────────────
  async function handleConfirmFinish(assignment: any, data: { actual_end: string; actual_start?: string; notes?: string }) {
    await confirmFinish.mutateAsync({
      shift_assignment_id: assignment.id,
      employee_id:         assignment.employee_id,
      shift_date:          format(new Date(assignment.shift_date), 'yyyy-MM-dd'),
      scheduled_start:     assignment.start_time?.slice(0, 5),
      scheduled_end:       assignment.end_time?.slice(0, 5),
  ...
  const BulkMoveModal = () => (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300 }}>
      <div style={{ background: 'white', borderRadius: '14px', width: '400px', padding: '1.75rem', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ background: 'var(--color-background-secondary)', borderRadius: '8px', padding: '10px 12px', marginBottom: '1.25rem', maxHeight: '120px', overflowY: 'auto' }}>
          {selectedAssignments.map((a: any) => (
            <div key={a.id} style={{ fontSize: '12px', color: 'var(--color-text-secondary)', marginBottom: '2px', display: 'flex', justifyContent: 'space-between' }}>
              <span>{a.first_name} {a.last_name}</span>
              <span style={{ color: 'var(--color-text-tertiary)' }}>{friendlyDate(format(new Date(a.shift_date), 'yyyy-MM-dd'))}</span>
            </div>
          ))}
        </div>
        <div className="form-group" style={{ marginBottom: '1.25rem' }}>
          <label className="form-label">Move all to this date</label>
          <input type="date" value={bulkMoveDate} min={weekKey} max={weekEnd} onChange={e => setBulkMoveDate(e.target.value)} />
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          <button onClick={executeBulkMove} disabled={!bulkMoveDate || bulkMoving}
          ...
          <button onClick={() => setConfirmBulkMove(false)} style={{ flex: 1, padding: '10px', fontSize: '13px', borderRadius: '8px' }}>Cancel</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="page" style={{ maxWidth: '100%' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {!isPublished && currentSchedule && (
            <button onClick={() => { setSelectMode(v => !v); clearSelection(); }}
              style={{ fontSize: '12px', padding: '6px 12px', background: selectMode ? '#1a1a18' : undefined, color: selectMode ? 'white' : undefined, border: selectMode ? 'none' : undefined, fontWeight: selectMode ? 500 : 400 }}>
              {selectMode ? '✓ Selecting' : 'Select shifts'}
            </button>
          )}
          {!currentSchedule && (
            <button className="btn-primary" disabled={createSchedule.isPending} onClick={() => createSchedule.mutate({ week_start: weekKey })}>
              {createSchedule.isPending ? 'Creating…' : '+ Create schedule'}
            </button>
          )}
          {currentSchedule?.status === 'draft' && (
            <button className="btn-gold" onClick={() => publishSchedule.mutate(currentSchedule.id)}>Publish schedule</button>
          )}
          {isPublished && <span className="badge badge-green">Published</span>}
        </div>
      </div>

      {/* Unconfirmed times banner */}
      {unconfirmedPastCount > 0 && !selectMode && (
        <div style={{ background: '#faeeda', border: '0.5px solid #ef9f27', borderRadius: '8px', padding: '10px 14px', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span style={{ fontSize: '16px' }}>⏱</span>
          <div style={{ flex: 1 }}>
            <span style={{ fontSize: '13px', fontWeight: 500, color: '#633806' }}>
              {unconfirmedPastCount} shift{unconfirmedPastCount !== 1 ? 's' : ''} from past days need finish times confirmed
            </span>
            <span style={{ fontSize: '12px', color: '#854f0b', marginLeft: '8px' }}>
              Click the ⚠ confirm badge on each card
            </span>
          </div>
        </div>
      )}
      ...
      {/* Select mode toolbar */}
      {selectMode && (
        <div style={{ background: '#1a1a18', borderRadius: '10px', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '1rem', flexWrap: 'wrap' }}>
          <div style={{ fontSize: '13px', color: 'white', fontWeight: 500 }}>
            {selectedIds.size === 0 ? 'Click a shift card to select it' : `${selectedIds.size} shift${selectedIds.size !== 1 ? 's' : ''} selected`}
          </div>
          <div style={{ display: 'flex', gap: '6px', marginLeft: 'auto', flexWrap: 'wrap' }}>
            {selectedIds.size > 0 && (
              <>
                <button onClick={() => setConfirmBulkMove(true)} style={{ fontSize: '12px', padding: '5px 12px', background: 'white', color: '#1a1a18', border: 'none', borderRadius: '6px', fontWeight: 500, cursor: 'pointer' }}>
                  Move {selectedIds.size} →
                </button>
                <button onClick={() => setConfirmBulkDelete(true)} style={{ fontSize: '12px', padding: '5px 12px', background: '#C41E3A', color: 'white', border: 'none', borderRadius: '6px', fontWeight: 500, cursor: 'pointer' }}>
                  Remove {selectedIds.size}
                </button>
                <button onClick={clearSelection} style={{ fontSize: '12px', padding: '5px 10px', background: 'transparent', color: 'rgba(255,255,255,0.5)', border: '0.5px solid rgba(255,255,255,0.2)', borderRadius: '6px', cursor: 'pointer' }}>Clear</button>
              </>
            )}
            {allAssignments.length > 0 && (
              <button onClick={selectedIds.size === allAssignments.length ? clearSelection : selectAll}
            ...
            <button onClick={exitSelectMode} style={{ fontSize: '12px', padding: '5px 10px', background: 'transparent', color: 'rgba(255,255,255,0.4)', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>Done</button>
          </div>
        </div>
      )}
      ...
      {!selectMode && (
        <div style={{ display: 'flex', gap: '12px', marginBottom: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '11px', color: 'var(--color-text-secondary)' }}>
            <div style={{ width: '10px', height: '10px', borderRadius: '3px', background: '#eaf3de', border: '0.5px solid #97c45960' }} />
            Confirmed ✓
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '11px', color: 'var(--color-text-secondary)' }}>
            <div style={{ width: '10px', height: '10px', borderRadius: '3px', background: '#faeeda', border: '0.5px solid #ef9f2760' }} />
            Needs confirmation
          </div>
          {!isPublished && currentSchedule && (
            <div style={{ marginLeft: 'auto', fontSize: '11px', color: 'var(--color-text-tertiary)' }}>
              💡 Drag to move · × to remove · ⚠ to confirm finish times
            </div>
          )}
        </div>
      )}
            ...
            <thead>
              <tr>
                {weekDates.map((date, i) => {
                  ...
                  const dayUnconfirmed = (assignmentsByDate[key] || []).filter((a: any) => isPast && !isToday && !actualsMap[a.id]?.is_confirmed).length;
                  ...
                  return (
                    <th key={i} style={{
                      ...
                      {!isClosed && hasWarn && !selectMode && <div style={{ fontSize: '9px', color: '#C41E3A', marginTop: '2px' }}>⚠ staffing</div>}
                      {!isClosed && dayUnconfirmed > 0 && !selectMode && <div style={{ fontSize: '9px', color: '#633806', marginTop: '1px' }}>⏱ {dayUnconfirmed} unconfirmed</div>}
                      {!isClosed && selectMode && dayCount > 0 && <div style={{ fontSize: '9px', color: daySelectedCount > 0 ? '#C41E3A' : 'var(--color-text-tertiary)', marginTop: '2px' }}>{daySelectedCount > 0 ? `${daySelectedCount}/${dayCount} selected` : `${dayCount} staff`}</div>}
                      {!isClosed && !selectMode && dayCount > 0 && !dayUnconfirmed && <div style={{ fontSize: '9px', color: 'var(--color-text-tertiary)', marginTop: '2px' }}>{dayCount} staff</div>}
                    </th>
                  );
                })}
              </tr>
                  ...
                  return (
                    <td key={i}
                      ...
                      style={{
                        ...
                        outlineOffset: '-2px', borderRadius: '4px', transition: 'background 0.1s', position: 'relative',
                      }}
                    >
                      {isClosed && (
                      ...
                      {!isClosed && dayAssignments.map((a: any) => {
                        const actual = actualsMap[a.id];
                        const isSelected = selectedIds.has(a.id);
                        // Past = before today. Today is NOT a past day (don't prompt during working day)
                        const shiftIsPast = isPast && !isToday;
                        ...
                        return (
                          <div key={a.id} style={{ position: 'relative' }}>
                            ...
                            <div
                              ...
                              style={{ paddingLeft: selectMode ? '4px' : 0 }}
                            >
                              <ShiftCard
                                assignment={a}
                                actual={actual}
                                isPublished={isPublished}
                                isPastDay={shiftIsPast}
                                ...
                                onConfirmFinish={() => setConfirmFinishAssignment(a)}
      ...
      {confirmRemove && (
        <ConfirmModal title="Remove this shift?" message={`Remove ${confirmRemove.name} from the ${confirmRemove.shiftName} on ${confirmRemove.date}?`}
      ...
      {confirmMove && (
        <ConfirmModal title="Move this shift?" message={`Move ${confirmMove.employeeName}'s ${confirmMove.shiftName} from ${friendlyDate(confirmMove.fromDate)} to ${friendlyDate(confirmMove.toDate)}?`}
          confirmLabel={moving ? 'Moving…' : 'Yes, move shift'} cancelLabel="Cancel"
          onConfirm={executeMove} onCancel={() => setConfirmMove(null)} />
      )}
      {confirmBulkDelete && (
        <ConfirmModal title={`Remove ${selectedIds.size} shift${selectedIds.size !== 1 ? 's' : ''}?`}
          message={`Permanently remove ${selectedIds.size} shift assignment${selectedIds.size !== 1 ? 's' : ''}. Cannot be undone.`}
          confirmLabel={bulkDeleting ? 'Removing…' : `Remove ${selectedIds.size}`} cancelLabel="Cancel" danger
      ...
      {confirmFinishAssignment && (
        <ConfirmFinishModal
          assignment={confirmFinishAssignment}
          existing={actualsMap[confirmFinishAssignment.id]}
          onSave={(data) => handleConfirmFinish(confirmFinishAssignment, data)}
          onUnconfirm={actualsMap[confirmFinishAssignment.id] ? () => unconfirmFinish.mutateAsync(confirmFinishAssignment.id) : undefined}
          onClose={() => setConfirmFinishAssignment(null)}
        />
      )}
    </div>
  );
